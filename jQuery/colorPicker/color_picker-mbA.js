...
$('.colorPicker').click(function(){
     ...
     $('.palette').hide();
     idPalette = $('#Palette-' + this.id);
     idTextValue = this.id;
         // $('#' + this.id).css('border-color', 'red');
          
          showColorPalette(idPalette);
...
function showColorPalette(idPalette) {
    
    idPalette.css('display', 'block');
...
let color = rgbToHex(e.target.style.backgroundColor);
  ...
  $('#' + idTextValue).val(color);
  $('#' + idTextValue).css('border-right', '15px solid ' + color );
  if(target === 'page'){
      $('.' + target + ' td').css('border-color', color);
  }else{
      $('.' + 'page' + ' td.' + target ).css('color', color);
  }
...
idPalette.onmouseover = () => {
  idPalette.mouseIsOver = true;

};

idPalette.onmouseout = () => {
  idPalette.mouseIsOver = false;
};

function hideColorPalette() {
  if(idPalette.mouseIsOver === false) {
    idPalette.css('display', 'none');
    console.log('#' + idTextValue);
    $('#' + idTextValue).css('border-color-right', '10px solid ' + $('#' + idTextValue).val());
...
const listOfInput = document.querySelectorAll('.colorPicker');
...
//for (let input of listOfInput) {
//input.style.borderRight =  `10px solid ${colorInput.value}`;
//}
